...
pygame.font.init()
myFont = pygame.font.SysFont("tahoma", 32)
...
class Grid:
    ...
    def guess(self, key):
        if self.selected_cube.n == 0 and possible(self.board, self.selected_cube.row, self.selected_cube.col, key):
            self.selected_cube.guess = key

    # update when number is confirmed 
    def update(self):
        if self.selected_cube.guess != 0:
...
class Cubes:
    ...
    def render(self, win):
        ...
        if self.n != 0:
            img = myFont.render(str(self.n), True, (0,0,0))
            img_rect = img.get_rect(center= (self.x_center, self.y_center))
            win.blit(img, img_rect)

        if self.guess != 0:
            img = myFont.render(str(self.guess), True, (150,150,150))
            img_rect = img.get_rect(center= (self.x_center, self.y_center))
            win.blit(img, img_rect)
